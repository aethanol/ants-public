...
class AntGame {
    ...
    deployAnt(antType, placeName) {
        try { //brute force error catching
            ...
            if(antType === "Queen") {      
                ant = Ants[antType].queen;    // TRYING SINGLETON idk why
            }
            else {
...
var Actions = {
    ...
    Inspire: class extends Action {
        act(_ant) {
            // get both ants
            var _enterAnt = _ant.place.entrance.ant;
            var _exitAnt = _ant.place.exit.ant;
            
            
            if(_enterAnt !== undefined && !_ant.inspired[_enterAnt.name]){
                _ant.inspired[_enterAnt.name] = _enterAnt;
                this.double(_enterAnt);  
            }
            
            if(_exitAnt !== undefined && !_ant.inspired[_exitAnt.name]){
                _ant.inspired[_exitAnt.name] = _exitAnt;
                this.double(_exitAnt);
            }
...
// create new unique Symbols for queen Singleton
...
var Ants = {
    ...
    Fire: class extends Ant {
        ...
        reduceArmor(amount) {
            this.action = 'Fire'; // set fire action only after it's been stung
