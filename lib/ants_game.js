    ...
    constructor(startingFood, numTunnels, tunnelLength, moatFrequency) {
        ...
        for (var tunnel = 0; tunnel < numTunnels; tunnel++) {
            ...
            for (var step = 0; step < tunnelLength; step++) {
                ...
                if (typeName === 'water') { // if the type is water, set water on place
                    curr.water = true;
                }
...
var Actions = {
    ...
    Inspire: class extends Action {
        act(ant) {
            // get both ants
            var _enterAnt = ant.place.enterance.ant;
            var _exitAnt = ant.place.exit.ant;
            
            if(!ant.inspired) { // if there are no inspired ants, add any ants near
                
                ant.inspired[_enterAnt.name] = _enterAnt;
                ant.inspired[_exitAnt.name] = _exitAnt;
                
                this.double(_enterAnt);
                this.double(_exitAnt);
            }
            else{ // check inspired obj, add
            
                if(!ant.inspired[_enterAnt.name]){
                    ant.inspired[_enterAnt.name] = _enterAnt;
                    this.double(_enterAnt);
                }
                
                if(!ant.inspired[_exitAnt.name]){
                    ant.inspired[_exitAnt.name] = _exitAnt;
                    this.double(_exitAnt);
                }             
            }           
        }
        // double the target ant's damage
        double(target){
            target.damage *= 2;
        }
    }
...
var Ants = {
    ...
    Fire: class extends Ant {
        ...
        reduceArmor(amount) {
            this.action = 'Fire'; // set fire action, otherwise it will do every turn
                                  // we only want when it's been stung
