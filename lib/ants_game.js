...
class Ant extends Insect {
  constructor(armor, cost, place){
    ...
    this._foodCost = cost;
    this._waterSafe = false; // default to false (scuba ant will change in it's constructor)
...
var action = {
    ...
    // throw action, get closest bee within specified range
    // reduce armor by amount specified by that ant
    Throw : function(ant, damage, range){
...
var Ants = {
  ...
  Grower : class extends Ant {
    constructor() {
      super(1,2);
      this._name = 'Grower';
      this._actBehavior = new Grow(this);
    }
    
    // act(colony) {    
    //   //every turn, act(this._colony) is called
  ...
  Hungry : class extends Ant {
      ...
      act() {
          ...
          // select a random bee to eat from the array
          //var target = this.place.getClosestBee(0,50);
          
          //bees[Math.floor(Math.random()*bees.length)];
          
          // if the 
        //   if(target){
        //       console.log(this + ' eats '+target + ' and will digest for 3 turns');
        //       // 
        //       target.reduceArmor(target.armor);
        //       this._hungry += 3;
        //   }else{
        //       // decrement turns until it can eat again
        //       this._hungry -= 1;
        //       // default do nothing act HERE TODO
        //   }
