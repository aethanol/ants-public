...
*/
class Insect {
    constructor(armor, place, behavior, action) {
        this._armor = armor;
        this._place = place;
        this._actName = 'Nothing'; // default to the nothing action strategy
        this._action = action;
        this._behavior = behavior;
        this._behavName = 'Nothing'; // default to the nothing behavior strategy
        
        this._watersafe = false; // all insects are not watersafe by default
        this._visible = true; // all insects are visible by default
    ...
    get behavName(){
        return this._behavName;
    }
    
    set behavName(name){
        this._behavName = name;
    }
...
class Ant extends Insect {
    ...
    // set action(action) {
    //     this._action = action;//new Actions[action](); // create new action object, and store it
    // }
...
class Water extends Place {
    constructor(name, exit, entrance) {
        super(name, exit, entrance);
        this._water = true;
    }
    
    // override _addIsect to check if the ant you are placing is watersafe
    _addInsect(insect) {
        if (insect instanceof Bee) {
            this._bees.push(insect);
            return true;
        }
        else if (this._ant === undefined && insect.watersafe) {
            this._ant = insect;
            return true;
        }
        else {
...
class AntColony {
    ...
    deployAnt(place, ant) {
        if (this._food >= ant.foodCost) {
            if (place.water && ant.behavior) { // only one invalid deployment condition for water
                return false;
            } else {
...
class AntGame {
    ...
    deployAnt(antType, placeName) {
        try { //brute force error catching
           ...
           // var ant = new Ants[antType](); // get the ant
            
            
            // if (ant.name === 'Queen' && ant.deployed === true) {
            //     console.log('There can only be one Queen!');
            //     return false;
            // }
            // else {
            //     ant = new Ants[antType]();
            // }
...
class AntFactory{
    ...
    createAnt(antType){
        //console.log('newAnt');
        var newAnt = new Ants[antType](); // create new ant dynamically
        newAnt.action = new Actions[newAnt.actName](); // assign the action
        newAnt.behavior = new Behaviors[newAnt.behavName](newAnt); // assign other behaviors
...
//     set container(boolean) {
//         this._ant.container = boolean;
//     }
    
//     set contained(ant) {
//         // check if the ant is already a container
//         if (ant.container) {
//             this._contained = ant;
//         }
...
//     Nothing : class extends Strategy {
        
//     },

//     Watersafe : class extends Strategy {
//         constructor(_ant){
            
//             _ant.watersafe = true; // set ants _watersafe property to be watersafe
            
//         }
        
//     },
    
//     Invisible : class extends Strategy {
//         constructor(ant){
//             ant.visible = false;
//         }
...
//     Container : class extends Strategy {
//         constructor(ant){
//             ant.container = true; // add a container 
//         }
...
var Ants = {
    ...
    Fire: class extends Ant {
        ...
        reduceArmor(amount) {
            this.actName = 'Fire'; // set fire action only after it's been stung
    ...
    Ninja: class extends Ant {
        ...
        constructor() {
            ...
            this.damage = 1;
            this.actName = 'Attack';
            this.behavName = 'Invisible'; // Ninjas are invisible -- kapow!
            //this.visible = false; 
        }
    ...
    Queen: class extends Ant {
        ...
        constructor(enforcer) {
           ...
           // this._deployed = false; // keep track of if the queen is deployed or not
        }
